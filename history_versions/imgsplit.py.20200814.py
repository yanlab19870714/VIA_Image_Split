...
def coord_adjust(polygon_list, positions, x1, y1):
    ...
    for pos in positions:
        ...
        polygon = shapely.affinity.translate(polygon, xoff = -x1, yoff = -y1)
                ...
                for i in range(overflow(img_w, box_w, overlap)):
                    for j in range(overflow(img_h, box_h, overlap)):
                        ...
                        new_img_path = os.path.join(
                            self.new_dir, f"{img_file_noJPG}_{i}_{j}.jpg")
                        ...
                        ## annotation_dict[f"{img_file_noJPG}_{i}_{j}"] = pgons_in_window
